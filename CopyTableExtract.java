...
trigger UpdateCaseActionCount on CaseAction__c (after insert, after update, after delete, after undelete) {
    ...
    if (!caseIds.isEmpty()) {
        ...
        // Store the Case Id and related CaseAction count in a map
        for (AggregateResult ar : caseActionCounts) {
            caseActionCountMap.put((Id) ar.get('Case__c'), (Integer) ar.get('expr0'));
        }
        ...
        for (Id caseId : caseIds) {
            ...
            Case caseRecord = new Case(
                Id = caseId,
                Count_Using_Trigger__c = actionCount
            );
...
private class RolloverStrategyTrigger_Test {
    ...
    static void RolloverStrategyTriggerTest() {
        ...
        System.runAs(testUser) {
            ...
            List<Rollover_Strategy__c> deletedRollOvrStrtgyList = [SELECT IsDeleted FROM Rollover_Strategy__c WHERE Id = :rollOvrStrtgyObj.Id ALL ROWS];
            system.assertEquals(1, deletedRollOvrStrtgyList.size(), 'Rollover Strategy should be found in the database.');
            system.assertEquals(true, deletedRollOvrStrtgyList[0].IsDeleted, 'Rollover Strategy should be marked as deleted.');
